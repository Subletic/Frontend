...
describe('WordComponent', () => {
  ...
  it('should set event listeners on ngAfterViewInit', () => {
    spyOn(component, 'setEventListeners');
  ...
  it('should emit events on space press', () => {
    ...
    const mockSelection = {
      ...
      getRangeAt: () => {
        ...
        if (mockWordSpan.firstChild) range.setStart(mockWordSpan.firstChild, 2);
        return range;
      },
    };
    ...
    expect(component.dataUpdate.emit).toHaveBeenCalledWith({
      changedWord: EXPECTED_WORD,
      idOfEmitter: 15,
    });
    expect(component.newWordAfter.emit).toHaveBeenCalledWith({
      wordAfter: 'st',
      idOfEmitter: 15,
    });
  });
  ...
  it('should emit events on backspace press at start', () => {
    ...
    expect(component.addSelfToPrevWord.emit).toHaveBeenCalledWith({
      idOfEmitter: 15,
    });
  });
  ...
  it('should not emit events on space press if WORD_BEFORE_CURSOR is empty', () => {
    ...
    const mockSelection = {
      ...
      getRangeAt: () => {
        const range = document.createRange();
        if (mockWordSpan.firstChild) range.setStart(mockWordSpan.firstChild, 0);
        return range;
      },
    };
